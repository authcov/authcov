...
class ConfigValidator {
  ...
  _validateButtonXPath() {
    const buttonXPath = this.config.buttonXPath;

    if(buttonXPath === undefined) {
      this._addErrorMessage('config.buttonXPath', 'must be defined');
      return;
    }
  }
  ...
  _validateApiEndpointsFile() {
    const apiEndpointsFile = this.config.apiEndpointsFile;

    if(apiEndpointsFile === undefined) {
      this._addErrorMessage('config.apiEndpointsFile', 'must be defined');
      return;
    }
  }

  _validatePagesFile() {
    const pagesFile = this.config.pagesFile;

    if(pagesFile === undefined) {
      this._addErrorMessage('config.pagesFile', 'must be defined');
      return;
    }
  }
  ...
  _validateReportPath() {
    ...
    if(reportPath === undefined) {
      this._addErrorMessage('config.reportPath', 'must be defined');
      return;
    }
  }
  ...
  _validateUnAuthorizedStatusCodes() {
    const statusCodes = this.config.unAuthorizedStatusCodes;
    const responseIsAuthorised = this.config.responseIsAuthorised;

    if(statusCodes !== undefined && responseIsAuthorised !== undefined) {
      console.log(chalk.yellow(
        "WARN: Both config.unAuthorizedStatusCodes and responseIsAuthorised() are defined, using responseIsAuthorised()"
      ));
    } else if(statusCodes === undefined && responseIsAuthorised === undefined) {
      this._addErrorMessage('config.unAuthorizedStatusCodes', 'must be defined or the function responseIsAuthorised() must be defined.');
      return;
    } else if(statusCodes !== undefined && responseIsAuthorised === undefined) {

      if(Array.isArray(statusCodes) === false) {
       // console.log(typeof(statusCodes))
        this._addErrorMessage('config.unAuthorizedStatusCodes', 'must be an array.');
        return;
      }

    } else if(statusCodes === undefined && responseIsAuthorised !== undefined) {

      if(typeof(responseIsAuthorised) !== 'function') {
        this._addErrorMessage('config.responseIsAuthorised', 'must be a function.');
        return;
      }
