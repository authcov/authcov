...
class ConfigValidator {
  ...
  _validateType() {
    ...
    if(!['spa', 'mpa'].includes(type)) {
      this._addErrorMessage('options.type', 'must be either "mpa" or "spa"');
      return;
    }
  }
  ...
  _validateAuthenticationType() {
    ...
    if(!['cookie', 'token'].includes(authenticationType)) {
      this._addErrorMessage('options.authenticationType', 'must be either "cookie" or "token"');
      return;
    }
  }
  ...
  _validateUnAuthorizedStatusCodes() {
    const statusCodes = this.config.options.unAuthorizedStatusCodes;
    const responseIsAuthorised = this.config.options.responseIsAuthorised;

    if(statusCodes !== undefined && responseIsAuthorised !== undefined) {
      console.log(chalk.yellow(
        "WARN: Both config.options.unAuthorizedStatusCodes and responseIsAuthorised() are defined, using responseIsAuthorised()"
      ));
    } else if(statusCodes === undefined && responseIsAuthorised === undefined) {
      this._addErrorMessage('options.unAuthorizedStatusCodes', 'must be defined or the function responseIsAuthorised() must be defined.');
      return;
    } else if(statusCodes !== undefined && responseIsAuthorised === undefined) {

      if(typeof(statusCodes) !== 'array') {
        this._addErrorMessage('options.unAuthorizedStatusCodes', 'must be an array.');
        return;
      }
      ...
      if(typeof(responseIsAuthorised) !== 'function') {
        this._addErrorMessage('options.responseIsAuthorised', 'must be a function.');
        return;
      }
