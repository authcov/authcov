...
class ApiEndpointData {
  ...
  saveToFile(fileName) {
    ...
    fs.writeFile(fileName, JSON.stringify(apiEndpointsData, null, 2), (error) => {
      if(error) {
  ...
  findAuthorisationHeadersForUsername(username) {
    ...
    this.config.options.authorisationHeaders.forEach((key) => {
  ...
  findIntrusionRequestsForUsername(username, intruderHeaders) {
    ...
    this.apiEndpoints.forEach((apiEndpoint) => {
      if(!apiEndpoint.usernamesRequested().includes(username)) {
        ...
        this.config.options.authorisationHeaders.forEach((key) => { delete originalHeaders[key] });
  ...
  async mpaPageResponseCallback(response, cookies, pageUrl, currentUser) {
    ...
    const responseAuthorised = this.config.responseIsAuthorised(response, responseBody);
    ...
    if(this.config.options.saveResponses === true) {
