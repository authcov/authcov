...
class ReportGenerator {
  constructor(apiEndpoints, pageData) {
  ...
  _createReportDir() {
    ...
    fs.mkdirSync('./tmp/report/pages');
    fs.mkdirSync('./tmp/report/screenshots');
  }
  ...
  _generateRequestPages() {
    this.apiEndpointsPresenter.apiEndpoints.forEach((apiEndpoint) => {
      ...
      if(fs.existsSync(dir)) {
        //return; //TODO: This is skipping the intrusion requests
  ...
  _generatePagePages() {
   this.pageData.pages.forEach((page) => {
      ...
      this._generateFile('./lib/reporter/views/page.ejs', `${this.reportPath}/pages/${page.id}.html`, data);
    });
  }
  ...
  _copyAssets() {
    ...
    fs.copyFile('./lib/reporter/views/popper.min.js', `${this.reportPath}/popper.min.js`, (err) => {
      if (err) throw err;
    });
