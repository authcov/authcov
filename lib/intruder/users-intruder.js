...
class UsersIntruder {
  ...
  async start() {
    ...
    for(let i = 0; i < this.config.options.intruders.length; i++) {
      let intruder = this.config.options.intruders[i];
  ...
  async _makeRequest(intruder, requestOptions, i, total) {
    return new Promise ((resolve, reject) => {
      request(requestOptions, (error, response, body) => {
        ...
        const responseAuthorised = this.config.options.responseIsAuthorised(response.statusCode, response.headers, body);
