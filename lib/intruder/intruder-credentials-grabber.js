...
class IntruderCredentialsGrabber {
  ...
  static async init(config) {
    const browser = await Browser.init({browserURL: 'http://localhost:9222'});
  ...
  async _getAuthHeadersCookie(username, password) {
    ...
    let url;
    if(this.config.options.cookiesTriggeringPage === undefined) {
      url = this.config.options.baseUrl;
    } else {
      url = this.config.options.cookiesTriggeringPage;
    }
