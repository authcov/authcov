...
class Crawler {
  ...
  async login(username, password) {
    ...
    await this.config.options.loginFunction(tab, username, password);
  ...
  handleRequest(request, sourceUrl) {
    ...
    const ignoreRequest = this.config.options.ignoreApiRequest(
      request.url(),
      request.method()
    );
  ...
  async startCrawling() {
    this.crawlPage(this.config.options.baseUrl, 0);
  }

  async crawlPage(url, depth) {
    if(depth > this.config.options.maxDepth) {
      return;
    }
    ...
    try {
      ...
      if (this.config.options.type == 'mpa' && typeof(this.apiEndpointData) == 'object' && typeof(this.apiEndpointData.apiResponseCallback) == 'function') {
      ...
      await pageEvents.waitForRequestsToFinish(this.config.options.xhrTimeout);

      this._verboseLog(`${url} - Finished waiting for XHR requests or they have all completed.`);
      await tab.waitFor(500);

      if(this.config.options.saveScreenshots === true) {
        try {
  ...
  processLink(links, depth) {
    links.forEach((link) => {
      link = this.resolveUrl(link, this.config.options.baseUrl);
  ...
  ignoreLink(url) {
    if(typeof(this.config) == 'object' && typeof(this.config.options.ignoreLink) == 'function') {
      return this.config.options.ignoreLink(url);
    } else {
      return false;
    }
  }
  ...
  _verboseLog(message) {
    if(this.config.options.verboseOutput === true) {
      console.log(message);
    }
  }
}
