...
class PageExplorer {
  constructor(page, pageUrl, currentUser, webAppConfig, pageData) {
    this.page = page;
  ...
  async getLinks() {
    ...
    //this.page.on('console', msg => console.log(`${this.pageUrl} => ${msg.text()}`));
    ...
    await this.page.exposeFunction('doNotClickButton', async (outerHTML) => {
      ...
      const ignoreButton = this.webAppConfig.ignoreButton(outerHTML);
    ...
    await this.page.$$eval(this.webAppConfig.buttonXPath, this._pageClickButtons);
