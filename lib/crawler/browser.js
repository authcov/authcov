...
class Browser {
  ...
  static async init(config) {
    const maxConcurrency = (config.options.maxConcurrency !== undefined) ? config.options.maxConcurrency : 10;
    let puppeteer_browser;
    let browserLaunched;

    if(config.options.browserURL !== undefined) {
      puppeteer_browser = await Puppeteer.connect({
        browserURL: config.options.browserURL
      });
      browserLaunched = false;

    } else if(config.options.browserWSEndpoint !== undefined) {
      puppeteer_browser = await Puppeteer.connect({
        browserWSEndpoint: config.options.browserWSEndpoint
      });
      browserLaunched = false;
